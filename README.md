...
UK Biobank and MSM data are access-controlled, so this repo ships code plus the tables in `Supp_tables/` only. Supply your own outputs (produced by [PGSC](https://github.com/reneemf/PGSC)) and update `data_dir` in [`config.R`](config.R) accordingly.
...
| Fig5 — MSM replication | `Fig5/Scripts/R2_PGSCs_amp_BioMe.R` | 
...
| `Supp_tables/Scripts/pop_counts.R` | `Supp_tables/pop_count/all_pop_counts.csv` | 
...
- **ED_Fig1** compares PGSC results against the GENIE GxC heritability and heteroskedasticity estimates from [Pazokitoroudi et al., *Am J Hum Genet* 2024](https://doi.org/10.1016/j.ajhg.2024.05.015).
